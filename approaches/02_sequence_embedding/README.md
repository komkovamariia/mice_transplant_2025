...
The historical `mirpy_analysis.ipynb` consumed derived Parquet artifacts, including a flattened aaV repertoire table and aaVJ count tables. Parquet therefore belongs to the sequence-space reanalysis layer, not to the original first pass.

Run the modular implementation only after `clean_clonotypes_aaV.parquet` has been prepared with traceable sample metadata:

...
The historical Study 2 notebook starts from `study2_data/clonosets_study2.csv` and repseq-compatible MiXCR clonotype tables, then consumes several derived CSV and Parquet result layers. The concise modular implementation below instead uses the shared flattened repertoire table.

Run it only after `clean_clonotypes_aaV.parquet` has been prepared with complete sample, mouse, tissue, and subtype annotations:

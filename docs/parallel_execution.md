...
Approach 1 delegates R/edgeR numerical work to the configured native thread environment. Count tables remain sample resolved; the six biological strata are analyzed serially so their R model objects do not compete for memory.
...
Approach 3 is dominated by pandas aggregation and CDR3-neighborhood indexing. The six biological strata are executed as separate notebook cells, allowing durable checkpoints without duplicating the canonical input table across multiple process pools.
